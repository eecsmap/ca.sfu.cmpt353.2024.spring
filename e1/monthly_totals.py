...
def pivot_months_pandas(data):
    ...
    """
    monthly = (
        data.groupby(["name", data["date"].dt.strftime("%Y-%m")])["precipitation"]
        .sum()
        .unstack(level=1)
    )
    counts = (
        data.groupby(["name", data["date"].dt.strftime("%Y-%m")])
        .size()
        .unstack(level=1)
    )
    # or
    ...
    # sum_aggregate_value = grouped_data.aggregate({"precipitation": "sum"}).reset_index()
    # count_aggregate_value = grouped_data.aggregate(
    #     {"precipitation": "count"}
    # ).reset_index()
    # monthly = sum_aggregate_value.pivot(
    #     index="name", columns="month", values="precipitation"
    # )
    # counts = count_aggregate_value.pivot(
    #     index="name", columns="month", values="precipitation"
    # )
